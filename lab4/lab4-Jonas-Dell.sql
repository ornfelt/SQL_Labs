...
/*
Foreign keys
Skriv queries (ALTER TABLE) för koppla ihop tabellerna med foreign_keys och ta med lämpliga val för
vad som ska hända vid updates och deletes på PK:
...
on update cascade
on delete cascade;
alter table departments drop constraint fk_departments_manager_id;
alter table projects drop constraint fk_projects_supervisor;
alter table project_members drop constraint fk_project_members_e_id;
alter table project_members drop constraint fk_project_members_p_id; */

alter table departments drop foreign key departments_ibfk_1;
alter table departments drop foreign key projects_ibfk_1;
alter table departments drop foreign key project_members_ibfk_1;
alter table departments drop foreign key project_members_ibfk_2;
