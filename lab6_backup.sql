...
foreign key(o_num) references Orders(number) on update cascade on delete cascade, foreign key(i_num) references Items(number) on update cascade on delete cascade);
...
create table customer_order(c_email varchar(25) not null, o_num int unsigned not null,
foreign key(c_email) references Customers(email) on update cascade on delete cascade,
...
/*Tänkte om gällande tabell för logs. Tidigare fanns det bara en log per item. Logs-tabellen behöver tömmas på innehåll för att denne ska kunna sammarbeta med
